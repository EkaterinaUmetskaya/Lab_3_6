...
uploadButton.addEventListener('click', async () => {
  ...
  try {
    ...
    const fileName = TextTraceEncoding.escapeHtml(TextTraceEncoding.fixFileName(result.fileName));
    ...
    resultContent.innerHTML = `
      ...
      <ul>${result.comparisons.map(item => `<li>${TextTraceEncoding.escapeHtml(TextTraceEncoding.fixFileName(item.fileName))} - ${item.similarityPercent}%</li>`).join('')}</ul>`;
  } catch (error) {
    alert(error.message);
  } finally {
    uploadButton.disabled = false;
    uploadButton.textContent = 'Проверить работу';
  }
});
