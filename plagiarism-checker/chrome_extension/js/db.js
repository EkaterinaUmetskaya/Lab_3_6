window.CourseGuardDB = (() => {
  const DB_NAME = window.ExtensionConfig?.dbName || 'docscope-ek-v1-indexeddb';
  const DB_VERSION = 2;
  ...
  async function saveDocumentResult(result) {
    ...
    const fixedName = window.CourseGuardEncoding.fixFileName(result.fileName);
  ...
  async function getDocuments() {
    ...
    return rows
      .map((row) => ({ ...row, fileName: window.CourseGuardEncoding.fixFileName(row.fileName) }))
...
window.ExtensionDB = window.CourseGuardDB;
