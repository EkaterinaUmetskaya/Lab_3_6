...
async function clearServerDocuments() {
  await ensureStorage();
...
async function router(req, res) {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/health') return sendJson(res, 200, { status: 'ok', service: 'DocScope EK standalone server', time: new Date().toISOString() });
    if (req.method === 'GET' && url.pathname === '/api/documents') return sendJson(res, 200, (await listRecords()).map(({ text, ...safe }) => safe));
    if (req.method === 'DELETE' && url.pathname === '/api/documents') { await clearServerDocuments(); return sendJson(res, 200, { ok: true }); }
...
await ensureStorage();
http.createServer(router).listen(config.port, () => console.log(`DocScope EK server started: http://localhost:${config.port}`));
